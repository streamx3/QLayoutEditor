...
RegType MainWindow::Index2RegType(int index) {
    if (index < 1)
        return RegType::CurUser;
    return RegType::System;
}

QString MainWindow::kbid2Name(QString kbid) {
    QString ret;
    for (auto it = m_kbids.begin(); it < m_kbids.end(); ++it) {
        auto pair = *it;
        if (pair.last() == kbid) {
            ret = pair.first();
            // qDebug().noquote() << "Pair for " << kbid << " is " << ret;
            return ret;
        }
    }
    return ret;
...
QString MainWindow::kbid2NameFuzzy(QString kbid) {
    auto findById = [this](const QString& candidate) -> QString {
        // for (const auto& row : m_kbids) {
        for (size_t i = 0; i < m_kbids.size(); ++i) {
            auto row = m_kbids[i];
            if (row.size() >= 2 && row[1].compare(candidate, Qt::CaseInsensitive) == 0)
                return row[0];
        }
        return {};
    };

    QString s = kbid.trimmed().toUpper();

    // Exact match first
    if (const QString exact = findById(s); !exact.isEmpty())
        return exact;

    const int n = s.size();
    if (n <= 3) // can't protect last 3 chars if string too short
        return {};

    const int maxPrefix = n - 3; // last 3 must remain unchanged
    ...
    for (int prefixLen = 1; prefixLen <= maxPrefix; ++prefixLen) {
        ...
        if (const QString hit = findById(candidate); !hit.isEmpty())
            return hit;
    }
